...
function NodeCG(bundle, socket) {
	 ...
	 * @name NodeCG#log
	 */
	this.log = new Logger(bundle.name);
	...
	if (process.env.browser) {
		...
		socket.on('error', function (err) {
			/* istanbul ignore else: Default handler. */
			if (err.type === 'UnauthorizedError') {
				var url = [location.protocol, '//', location.host, location.pathname].join('');
				location.href = '/authError?code=' + err.code + '&message=' + err.message + '&viewUrl=' + url;
			} else {
...
NodeCG.prototype.listenFor = function (messageName, bundleName, handler) {
	...
	for (var i = 0; i < len; i++) {
		...
		if (messageName === existingHandler.messageName && bundleName === existingHandler.bundleName) {
			throw new Error(format('%s attempted to declare a duplicate "listenFor" handler:',
